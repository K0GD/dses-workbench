...
The development history is mirrored read-only at <https://github.com/dses-science/dses-workbench> in the DSES GitHub organization (same arrangement as the EVE modem's `dses-science/eve-modem`; mirrored since 2026-09-22, in the organization since 2026-09-23). The NAS bare repo stays the master; the mirror carries `main` and every release tag with the private working notes (`CLAUDE.md`) removed from every commit by `git filter-repo`. After the release commit and tag are pushed to origin:
